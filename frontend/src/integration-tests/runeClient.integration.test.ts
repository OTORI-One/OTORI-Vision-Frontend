...
// Add Jest imports for TypeScript
import { describe, expect, test, beforeAll } from '@jest/globals';

// Constants
const OVT_RUNE_ID = '240249:101';
const BACKEND_URL = process.env.RUNE_API_URL || 'http://localhost:3030';
...
(SHOULD_RUN_INTEGRATION_TESTS ? describe : describe.skip)('RuneClient Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Create client with real backend if available, mock mode if not
    runeClient = new RuneClient({
      baseUrl: BACKEND_URL,
      mockData: !backendAvailable
    });
    
    // Log test mode
    console.log(`Running RuneClient tests in ${backendAvailable ? 'REAL' : 'MOCK'} mode`);
  });
  ...
  test('should calculate price impact correctly', async () => {
    ...
    expect(sellImpact1000).toBeLessThan(buyImpact1000);
